"""FastAPI application for deepseek-finetune-toolkit."""
...
app = FastAPI(title="deepseek-finetune-toolkit", version="0.1.0", description="deepseek-finetune-toolkit API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "deepseek-finetune-toolkit", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "deepseek-finetune-toolkit", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "deepseek-finetune-toolkit", "version": "0.1.0", "ready": True,
